...
function _M:init(actor)
	self.actor = actor
	Dialog.init(self, "Game Over", 500, 500)

	self:generateList()

	local stuff = [[You are #LIGHT_RED#destroyed#LAST#!  Your mind has been shattered, and you are left a gibbering wreck, consumed by your guilty conscience.

You can view some info about your character and save the data to file, or abandon all memory and leave this place.
]]
...
No, not this time...
	...
	self.c_desc = Textzone.new{width=self.iw, auto_height=true, text = stuff}
...
function _M:use(item)
   ...
   if act == "dump" then
      game:registerDialog(require("mod.dialogs.CharacterSheet").new(game.player))
